...
#' @param repe Numeric or complex vector with repetitions
...
FAT2DIC.ad=function(f1,
  ...
  anava1$Df[4]=1
  anava1$`Sum Sq`[4]=anava1$`Sum Sq`[4]-sum(anava1$`Sum Sq`[c(1:3)])
  anava1$`Mean Sq`[4]=anava1$`Sum Sq`[4]/anava1$Df[4]
  anava1$`F value`[1:4]=anava1$`Mean Sq`[1:4]/anava1$`Mean Sq`[5]
  for(i in 1:nrow(anava1)-1){
    anava1$`Pr(>F)`[i]=1-pf(anava1$`F value`[i],anava1$Df[i],anava1$Df[5])
  }
  ...
  { cat(green(bold("-----------------------------------------------------------------\n")))
    ...
    {cat(green(bold("\n-----------------------------------------------------------------\n")))
      ...
      if(quali[i]==TRUE){
        ## Tukey
        if(mcomp=="tukey"){
          letra <- HSD.test(resp, fatores[,i], anava$Df[5],anava$`Mean Sq`[5], alpha=alpha.t)
        ...
        if(mcomp=="lsd"){
          letra <- LSD.test(resp, fatores[,i], anava$Df[5],anava$`Mean Sq`[5],alpha=alpha.t)
          letra1 <- letra$groups; colnames(letra1)=c("resp","groups")
          if(transf !=1){letra1$respo=tapply(response,fatores[,i],mean, na.rm=TRUE)[rownames(letra1)]}}
        if (mcomp == "sk"){
          letra=sk_triple(resp, fatores[,i], anava$Df[5],anava$`Sum Sq`[5],alpha.t)
          letra1 <- letra; colnames(letra1)=c("resp","groups")
          if(transf !=1){letra1$respo=tapply(response,fatores[,i],mean, na.rm=TRUE)[rownames(letra1)]}}
        if(mcomp=="duncan"){
          letra <- duncan.test(resp, fatores[,i],anava$Df[5],anava$`Mean Sq`[5], alpha=alpha.t)
          letra1 <- letra$groups; colnames(letra1)=c("resp","groups")
          if(transf !=1){letra1$respo=tapply(response,fatores[,i],mean, na.rm=TRUE)[rownames(letra1)]}}
        print(letra1)
        ...
        if(CV==TRUE){grafico=grafico+labs(caption=paste("p-value = ", if(anava$`Pr(>F)`[i]<0.0001){paste("<", 0.0001)}
                                                        ...
                                                        round(abs(sqrt(anava$`Mean Sq`[5])/mean(c(resp,respAd),na.rm=TRUE))*100,2),"%"))}
    ...
    if(quali[1]==TRUE & quali[2]==TRUE){
      ...
      if (mcomp == "tukey"){
        ...
        for (i in 1:nv2) {
          ...
          tukey=HSD.test(respi,trati,anava$Df[5],anava$`Mean Sq`[5],alpha.t)
      ...
      if (mcomp == "duncan"){
        ...
        for (i in 1:nv2) {
          trati=fatores[, 1][Fator2 == lf2[i]]
          respi=resp[Fator2 == lf2[i]]
          duncan=duncan.test(respi,trati,anava$Df[5],anava$`Mean Sq`[5],alpha.t)
      ...
      if (mcomp == "lsd"){
        ...
        for (i in 1:nv2) {
          trati=fatores[, 1][Fator2 == lf2[i]]
          respi=resp[Fator2 == lf2[i]]
          lsd=LSD.test(respi,trati,anava$Df[5],anava$`Mean Sq`[5],alpha.t)
      ...
      if (mcomp == "sk"){
        ...
        for (i in 1:nv2) {
          ...
          respi=resp[Fator2 == lf2[i]]
          sk=sk_triple(respi,trati,anava$Df[5],anava$`Sum Sq`[5],alpha.t)
