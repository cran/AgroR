  ...
  if (as.numeric(tab[5, 5]) <= alpha.f) {
    ...
    if(quali[1]==TRUE & quali[2]==TRUE){
      if (mcomp == "tukey"){
        ...
        ordem=c()
        for (i in 1:nv2) {
          tukey=HSD.test(resp[fat[,2]==l2[i]], fat[,1][fat[,2]==l2[i]], num(tab.f1f2[nv2+1,1]),
      ...
      if (mcomp == "duncan"){
        ...
        ordem=c()
        for (i in 1:nv2) {
          duncan=duncan.test(resp[fat[,2]==l2[i]], fat[,1][fat[,2]==l2[i]], num(tab.f1f2[nv2+1,1]),
                             num(tab.f1f2[nv2+1,2])/num(tab.f1f2[nv2+1,1]), alpha.t)
    ...
    if(quali[1]==TRUE && quali[2]==TRUE){
      ...
      if (mcomp == "duncan"){
        duncangrafico1=c()
        for (i in 1:nv1) {
          duncan=duncan.test(resp[fat[, 1] == l1[i]], fat[,2][fat[, 1] == l1[i]], num(tab.f2f1[nv1 +1, 1]),
                             num(tab.f2f1[nv1 + 1, 2])/num(tab.f2f1[nv1 +1, 1]),alpha.t)
          colnames(duncan$groups)=c("resp","groups")
      ...
      if (mcomp == "lsd"){
        lsdgrafico1=c()
        for (i in 1:nv1) {
          lsd=LSD.test(resp[fat[, 1] == l1[i]], fat[,2][fat[, 1] == l1[i]], num(tab.f2f1[nv1 +1, 1]),
                             num(tab.f2f1[nv1 + 1, 2])/num(tab.f2f1[nv1 +1, 1]),alpha.t)
          colnames(lsd$groups)=c("resp","groups")
    ...
    if(quali[1]==FALSE  && color=="gray"| quali[2]==FALSE && color=="gray"){
      if(quali[2]==FALSE){
        ...
        grafico=polynomial2(fator2,resp,fator1, grau = grau,color=color1,ylab=ylab,xlab=xlab,
                    theme=theme,point=point,
                    posi= posi,ylim=ylim,
                    textsize=textsize, se=errorbar, family=family)}
      if(quali[2]==TRUE){
        fator1=as.numeric(as.character(fator1))
        grafico=polynomial2(fator1,resp,fator2, grau = grau,color=color1,ylab=ylab,xlab=xlab,
                    theme=theme,point=point,
                    posi = posi,ylim=ylim,
                    textsize=textsize, se=errorbar, family=family)}
    }
    if(quali[1]==FALSE  && color=="rainbow"| quali[2]==FALSE && color=="rainbow"){
      if(quali[2]==FALSE){
        fator2=as.numeric(as.character(fator2))
        grafico=polynomial2_color(fator2,resp,fator1, grau = grau,color=color1,ylab=ylab,xlab=xlab,
                    theme=theme,point=point,
                    posi= posi,ylim=ylim,
                    textsize=textsize, se=errorbar, family=family)}
      if(quali[2]==T){
        fator1=as.numeric(as.character(fator1))
        grafico=polynomial2_color(fator1,resp,fator2, grau = grau,color=color1,ylab=ylab,xlab=xlab,
                    theme=theme,point=point,
                    posi = posi,ylim=ylim,
                    textsize=textsize, se=errorbar, family=family)}
    }
