#' Analysis: Analysis of Variance of Aligned Rank Transformed Data in FAT2DBC
...
FAT2DBC.art=function(f1,
  ...
  data=data.frame(fator1,fator2,bloco,response)
  mod=art(response~fator1+fator2+fator1:fator2+(1|bloco),data=data)
  anova(mod, response="aligned")
  summary(mod)
  cat(green(bold("\n-----------------------------------------------------------------\n")))
  cat(green(italic("Analysis of Variance of Aligned Rank Transformed Data")))
  cat(green(bold("\n-----------------------------------------------------------------\n")))
  an=data.frame(anova(mod));colnames(an)=c("FV","F","Df","Df.res","p-value")
  print(an)
  a=anova(mod)
  ...
  .group=a1$.group
  g1=ggplot(a1,aes(x=fator1,y=emmean))+
